...
import com.ictproject.moviemate.domain.user.dto.KakaoDeleteResponseDTO;
...
public class KakaoUserService {
    ...
    // 카카오 회원탈퇴
    public void deleteKakaoUser(HttpSession session) {

        KakaoDeleteResponseDTO dto = kakaoDeleteUser(session);
    ...
    public KakaoDeleteResponseDTO kakaoDeleteUser(HttpSession session) {
        String requestUri = "https://kapi.kakao.com/v1/user/unlink";
        ...
        RestTemplate template = new RestTemplate();
        ResponseEntity<KakaoDeleteResponseDTO> responseEntity = template.exchange(
                requestUri,
                HttpMethod.POST,
                new HttpEntity<>(params, headers),
                KakaoDeleteResponseDTO.class
        );

        KakaoDeleteResponseDTO responseJSON = responseEntity.getBody();
        log.info("응답 id: {}", responseJSON);
